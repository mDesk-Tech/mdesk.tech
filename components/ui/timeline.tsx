...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  useEffect(() => {
    ...
    const update = () => {
      // Use scrollHeight to better reflect dynamic content growth
      setHeight(el.scrollHeight || el.getBoundingClientRect().height);
    };

    update();

    // Observe size changes to avoid layout jumps when content expands
  ...
  return (
    <div
      className="w-full bg-white dark:bg-neutral-950 font-sans md:px-10 no-anchor"
      ref={containerRef}
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-linear-to-t from-cyan-500 via-teal-500 to-transparent from-0% via-10% rounded-full"
          />
        </div>
      </div>
    </div>
